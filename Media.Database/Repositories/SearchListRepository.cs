    ...
    ILogger<SearchListRepository> logger) : ISearchListRepository
{
    ...
    public async Task<SearchList?> AddAsync(
        OwnerScope scope, int ownerId, SearchListType listType, string name, IReadOnlyList<SearchListLine> lines)
    {
        try
        {
            ...
            await WriteContentAsync(scope, ownerId, skeleton.Id, name, lines);
    ...
    public async Task<SearchList?> GetAsync(OwnerScope scope, int ownerId, Guid uuid)
    {
        try
        {
            ...
            var content = await _cqlExecutor.QuerySingleAsync(
                ...
                p =>
                {
                    p.Add(scope == OwnerScope.Person ? pn.PersonId : pn.GroupId, ownerId);
                    p.Add(scope == OwnerScope.Person ? pn.PersonSearchListId : pn.GroupSearchListId, skeleton.Id);
                },
        ...
        OwnerScope scope, int ownerId, SearchListType? listType = null)
    {
        try
        {
            ...
            var contents = await _cqlExecutor.QueryManyAsync(
                ...
                p => p.Add(scope == OwnerScope.Person ? pn.PersonId : pn.GroupId, ownerId),
                row => scope == OwnerScope.Person
                    ? row.ToPersonSearchListContent()
                    : row.ToGroupSearchListContent());
    ...
    public async Task<SearchList?> UpdateAsync(
        OwnerScope scope, int ownerId, Guid uuid, SearchListType listType, string name, IReadOnlyList<SearchListLine> lines)
    {
        try
        {
    ...
    public async Task<bool> DeleteAsync(OwnerScope scope, int ownerId, Guid uuid)
    {
        try
        {
            ...
            await _cqlExecutor.ExecuteAsync(
                ...
                {
                    p.Add(scope == OwnerScope.Person ? pn.PersonId : pn.GroupId, ownerId);
                    p.Add(scope == OwnerScope.Person ? pn.PersonSearchListId : pn.GroupSearchListId, id.Value);
                });
    ...
    private Task WriteContentAsync(
        OwnerScope scope, int ownerId, int id, string name, IReadOnlyList<SearchListLine> lines)
    {
        ...
        return _cqlExecutor.ExecuteAsync(
            ...
            {
                p.Add(scope == OwnerScope.Person ? pn.PersonId : pn.GroupId, ownerId);
                p.Add(scope == OwnerScope.Person ? pn.PersonSearchListId : pn.GroupSearchListId, id);
                p.Add(pn.Name, _cipher.Encrypt(name));
                p.Add(pn.Payload, _cipher.Encrypt(payload));
                p.Add(pn.PayloadVersion, SearchList.CurrentPayloadVersion);
            });
    }
