...
public record SourceMachineRegistrations
{
    ...
    /// API key (X-API-KEY) — it is never serialized to a client; a device instead learns it via
    /// the one-time vault-reveal link (see <see cref="VaultToken"/>). See
    ...
    /// <summary>
    /// Gets the one-time vault-reveal token, minted when the source machine is first created.
    /// Visiting <c>GET /api/registration/vault/{token}</c> with this value returns
    /// <see cref="SourceMachineUuid"/> exactly once, then permanently invalidates the token. See
    /// <see cref="RegistrationId"/> for why this isn't <c>required</c>.
    /// </summary>
    [JsonIgnore]
    public Guid VaultToken { get; init; }

    /// <summary>
    /// Gets the timestamp after which <see cref="VaultToken"/> is no longer honored, even if
    /// never consumed. See <see cref="RegistrationId"/> for why this isn't <c>required</c>.
    /// </summary>
    [JsonIgnore]
    public DateTimeOffset VaultTokenExpiresOn { get; init; }

    /// <summary>
    /// Gets the timestamp <see cref="VaultToken"/> was consumed, or null if it has not been
    /// viewed yet. See <see cref="RegistrationId"/> for why this isn't <c>required</c>.
    /// </summary>
    [JsonIgnore]
    public DateTimeOffset? VaultTokenConsumedOn { get; init; }
}
